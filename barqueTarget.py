        ...
        if self.resource_type == "ct":
            ...
            try:
                copyfile("".join((self.destination, self.file_target, ".conf")),
                         '/etc/pve/nodes/{}/qemu-server/{}.conf'.format(
                             self.host, self.vmid))
            except:
                return True, "Unable to copy config file"
            return False, "vm config file copied successfully"
    ...
    def update_config_disk(self):
        ...
        try:
            for line in config_file:
                if line.startswith("["): # snapshots are deleted, no need to include them in the new config
                    break
                if "rootfs" in line:
                    sub = line.split(':')
                    newline = sub[0] + ':' + sub[1] + ':' + \
        ...
        subprocess.check_output(
            "rbd snap purge {}/{}".format(self.ceph_pool, self.ceph_vmdisk),
            shell=True)

        return False, "config file disk size modified"


    def update_config_ostype(self):
        """
        For use with migration, re: tasks.skysilk.com/issues/3338
        Determines the original ostype configuration option from the config file
        stored with the backup, then sets this option in the new config file.
        """
        ...
        try:
            for line in backup_config_file:
                if "ostype" in line:
                    config_ostype = line.split(': ')[1]
        except Exception:
            backup_config_file.close()
            return True, "Error parsing config files"
        # Close the file
        backup_config_file.close()
        # Check if ostype paramater has been retrieved
        if config_ostype == "":
            return True, "Error retrieving ostype from backup conf file"

        # read present config file
        try:
            config_file = open(
                "/etc/pve/nodes/{}/lxc/{}.conf".format(self.host, self.vmid),
                "r")
        ...
        try:
            for line in config_file:
                if line.startswith("["):
                    # snapshots are deleted, no need to include them in the new config
                    break
                if "ostype" in line:
                    newline = 'ostype: ' + config_ostype
                    line_buffer.append(newline)
                else:
                    line_buffer.append(line)
        except Exception:
        ...
        return False, "config file ostype modified"
